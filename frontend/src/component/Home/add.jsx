...
export const Add = (props) => {
  ...
  const addNewItem = async (event) => {
    ...
    if (initValue.email !== "" && !onlyEmail.test(initValue.email)) { 
      return setInitError({ 
    ...
    if (!onlyNum.test(initValue.phone)) { 
      return setInitError({ 
        phoneError: 'Please enter a valid phone number' 
      }); 
    }
    if (onlyNum.test(initValue.phone) && !digiPh.test(initValue.phone)) { 
      return setInitError({ 
        phoneError: 'Please enter a valid 10 digit mobile number' 
      }); 
    }
    if (onlyNum.test(initValue.phone) && !fZeroNAllow.test(initValue.phone)) { 
      return setInitError({ 
        phoneError: 'Please enter a valid phone number' 
      }); 
    }
    ...
    try {
      ...
      if(res.data.msg === "Email already exists") { 
        return setInitMainError(res.data.msg); 
      }
      if(res.data.msg === "Phone no already exists") { 
        return setInitMainError(res.data.msg); 
      }
      setInitError({ 
        name: '', 
    ...
    <div className="popup">
      <div className="popup-inner">
        ...
        <div className="popup-details">
          {initMainError && (
            <p className="app-error-lst">{initMainError}</p>
          )}
          <form 
            onSubmit={addNewItem} 
            className="app-frms">
